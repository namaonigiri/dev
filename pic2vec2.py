...
import base64
import requests
from function import stream, process
...
def main():
    #入力すべき事項
    n = [1, 3, 4, 8, 11, 12, 13, 14, 17, 19, 20, 22, 25, 26, 27, 31, 33, 37, 39, 40] #画像
    m = 5 #終わった数
    name = "Honoka"
    persona_path = "./data/hinata/hinata.txt"#ペルソナテキストのパス
    picture_path = "./data/picture"#写真のディレクトリ
    ...
    for j in range(1):
        output_path = f"./output/pic2vec{j}.csv"
        for i in range(m, 20):
            #写真をデコード
            image_path = f"{picture_path}/{n[i]}.jpg"
